...
kitchen.order('Yokohama Mama').then(console.log);
...
kitchen.order('Chimichanga').then(console.log).catch(console.log);
...
//  Also add a method to catch an error if something goes wrong
 ...
 kitchen.addToMenu(newBurger).then(console.log).catch(console.log).then(kitchen.readMenu().then(console.log))
...
Promise.all(customers.map((people) => {
  kitchen.order(people.order).then(console.log).catch(console.log);
}))
